    ...
    try:
        ...
        try:
            ...
            if _is_rate_limit_error(e):
                use_fallback = True
                google_error_code = "GOOGLE_ADK_RATE_LIMIT"
                # Mark Google ADK step as failed due to rate limit
                skipped_step = tools["create_adk_run_step"](
                    run_id=run_id,
                    step="google_adk",
                    status="failed",
                    error=error_message,
                    error_code=google_error_code,
                ...
                google_error_code = "GOOGLE_ADK_EXCEPTION"
        ...
        if use_fallback:
            fallback_step = tools["create_adk_run_step"](
                run_id=run_id,
                step="fallback",
                status="success",
                data={
                    "source": "manual_agents",
                    "reason": "google_adk_failed",
                    "google_error_code": google_error_code,
                },
            )
            fallback_step_id = fallback_step.get("id")
            if fallback_step_id:
                tools["finish_adk_run_step"](fallback_step_id)
